...
class AdminJobController extends AdminbaseController {
	...
	function add(){
		$terms = $this->terms_model->where(array("taxonomy"=>"job"))->order(array("listorder"=>"asc"))->select();
	...
	function add_post(){
		if (IS_POST) {
			if(empty($_POST['post']['zp_zhize'])){
				$this->error("工作职责不能为空！");
			}
			if(empty($_POST['post']['zp_gangwei'])){
				$this->error("岗位要求不能为空！");
			}
			if(empty($_POST['term'])){
			...
			$article['zp_zhize']=htmlspecialchars_decode($article['zp_zhize']);
			$article['zp_gangwei']=htmlspecialchars_decode($article['zp_gangwei']);
			$article['zp_xssm']=htmlspecialchars_decode($article['zp_xssm']);

			$result=$this->job_model->add($article);
			if ($result) {
				//
	...
	public function edit(){
		...
		$post=$this->job_model->where("id=$id")->find();
	...
	public function edit_post(){
		if (IS_POST) {
			...
			$article['zp_gangwei']=htmlspecialchars_decode($article['zp_gangwei']);
			$article['zp_xssm']=htmlspecialchars_decode($article['zp_xssm']);
			$result=$this->job_model->save($article);
			if ($result!==false) {
				$this->success("保存成功！");
			} else {
				$this->error("保存失败！");
	...
	private  function _lists($status=1){
		...
		$job=$this->term_relationships_model
		->alias("a")
		->join(C ( 'DB_PREFIX' )."posts b ON a.object_id = b.id")
		->where($where)
	...
	private function _getTree(){
		...
		$result = $this->terms_model->where(array('taxonomy'=>'job'))->order(array("listorder"=>"asc"))->select();
		...
		if(isset($_POST['ids'])){
			...
			if($status!==false){
				foreach ($_POST['ids'] as $post_id){
					...
					if(empty($count)){
						$status=$this->job_model->where(array("id"=>$post_id))->delete();
					}
				}
			...
			if(isset($_GET['id'])){
				...
				if($status!==false){
					...
					if(empty($count)){
						$status=$this->job_model->where("id=$id")->delete();
					}
