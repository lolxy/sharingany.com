...
class AdminProductController extends AdminbaseController {
	...
	function add_post(){
		if (IS_POST) {
			...
			$article['product']=htmlspecialchars_decode($article['product']);
	...
	private  function _lists($status=1){
		...
		$product=$this->term_relationships_model
		->alias("a")
		->join(C ( 'DB_PREFIX' )."posts b ON a.object_id = b.id")
		->where($where)
	...
	function check(){
		if(isset($_POST['ids']) && $_GET["check"]){
			...
			$ids=join(",", $ids);
			if ( $this->product_model->where("id in ($ids)")->save($data)!==false) {
				$this->success("审核成功！");
			} else {
				$this->error("审核失败！");
			}
		...
		if(isset($_POST['ids']) && $_GET["uncheck"]){
			...
			$ids=join(",", $ids);
			if ( $this->product_model->where("id in ($ids)")->save($data)) {
				$this->success("取消审核成功！");
			} else {
				$this->error("取消审核失败！");
			}
		...
		if(isset($_POST['ids'])){
			...
			if($status!==false){
				foreach ($_POST['ids'] as $post_id){
					...
					if(empty($count)){
						$status=$this->product_model->where(array("id"=>$post_id))->delete();
					}
				}
			...
			if(isset($_GET['id'])){
				...
				if($status!==false){
					...
					if(empty($count)){
						$status=$this->product_model->where("id=$id")->delete();
					}
