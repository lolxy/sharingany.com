...
class AdminCaseController extends AdminbaseController {
	...
	function _initialize() {
		parent::_initialize();
		$this->case_model = D("Portal/Posts");
	...
	function add(){
		$terms = $this->terms_model->where(array("taxonomy"=>"anli"))->order(array("listorder"=>"asc"))->select();
	...
	function add_post(){
		if (IS_POST) {
			...
			$article['case']=htmlspecialchars_decode($article['case']);
			$article['functionss']=htmlspecialchars_decode($article['functionss']);
			$article['price']=htmlspecialchars_decode($article['price']);
			$article['flow']=htmlspecialchars_decode($article['flow']);

			$result=$this->case_model->add($article);
			if ($result) {
				//
	...
	public function edit(){
		...
		$post=$this->case_model->where("id=$id")->find();
	...
	public function edit_post(){
		if (IS_POST) {
			...
			$article['price']=htmlspecialchars_decode($article['price']);
			$article['flow']=htmlspecialchars_decode($article['flow']);
			$result=$this->case_model->save($article);
			if ($result!==false) {
				$this->success("保存成功！");
			} else {
				$this->error("保存失败！");
	...
	private  function _lists($status=1){
		$term_id=4;
		$termid = sp_get_child_terms($term_id);
		foreach($termid as $vo){
			$termids[]=$vo['term_id'];
			}
		$term_id = empty($termids)?$term_id:$term_id.','.implode(',',$termids);
		...
		$where_ands=empty($term_id)?array("a.status=$status"):array("a.term_id in ($term_id) and a.status=$status");
		...
		$case=$this->term_relationships_model
		->alias("a")
		->join(C ( 'DB_PREFIX' )."posts b ON a.object_id = b.id")
		->where($where)
    	...
    	$terms = $this->terms_model->order(array("term_id in ($term_id)"))->getField("term_id,name",true);
	...
	private function _getTree(){
		...
		$result = $this->terms_model->where(array('taxonomy'=>'anli'))->order(array("listorder"=>"asc"))->select();
	...
	function check(){
		if(isset($_POST['ids']) && $_GET["check"]){
			...
			$ids=join(",", $ids);
			if ( $this->case_model->where("id in ($ids)")->save($data)!==false) {
				$this->success("审核成功！");
			} else {
				$this->error("审核失败！");
			}
		...
		if(isset($_POST['ids']) && $_GET["uncheck"]){
			...
			$ids=join(",", $ids);
			if ( $this->case_model->where("id in ($ids)")->save($data)) {
				$this->success("取消审核成功！");
			} else {
				$this->error("取消审核失败！");
			}
		...
		if(isset($_POST['ids'])){
			...
			if($status!==false){
				foreach ($_POST['ids'] as $post_id){
					...
					if(empty($count)){
						$status=$this->case_model->where(array("id"=>$post_id))->delete();
					}
				}
			...
			if(isset($_GET['id'])){
				...
				if($status!==false){
					...
					if(empty($count)){
						$status=$this->case_model->where("id=$id")->delete();
					}
